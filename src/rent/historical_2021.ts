...
async function extractInternalData(url: string) {
  try {
    ...
    return {
      ...
      suite: document.querySelector('.feature__extra-info')?.textContent?.trim()?.replace(/[^0-9-]/g, ''),
...
async function getAllData() {
  const urls = getDataByYearRent(2021)
  const data: any[] = []
  ...
  await writeFile(resolve('./error_pages_rent_2021.txt'), errorPages.toString(), { encoding: 'utf-8' })
  await new ObjectsToCsv(data).toDisk('./final_rent_2021.csv')
}
