...
async function extractData(document: Document, pageUrl: string) {
  ...
  return await Promise.all(containers.map(async (el) => {
    ...
    return {
      ...
      area: details.filter(item => item?.includes('m²'))[0]?.replace(/[^0-9-]/g, ''),
      ...
      ...await extractInternalData('https://web.archive.org' + el.querySelector('a.property-card__content-link')?.getAttribute('href')),
...
async function extractInternalData(url: string) {
  try {
    ...
    return {
      condo: document.querySelector('span.price__list-value.condominium')?.textContent?.replace(/\D/g, '').trim(),
      suite: document.querySelector('.features__extra-info')?.textContent?.trim()?.replace(/[^0-9-]/g, ''),
...
async function getAllData() {
  const urls = getDataByYearRent(2022)
  const data: any[] = []
  ...
  await writeFile(resolve('./error_pages__rent_2022.txt'), errorPages.toString(), { encoding: 'utf-8' })
  await new ObjectsToCsv(data).toDisk('./final_rent_2022.csv')
}
