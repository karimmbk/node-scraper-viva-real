...
async function getAllData() {
  const urls = getDataByYearRent(2020)
  const data: any[] = []
  ...
  await writeFile(resolve('./error_pages_rent_2020.txt'), errorPages.toString(), { encoding: 'utf-8' })
  await new ObjectsToCsv(data).toDisk('./final_rent_2020.csv')
}
